...
use crate::forest::{Forest, unify_tree};
...
impl Grammar {
  ...
  fn parse(&self, input: &[&str]) -> Vec<NodeRef> {
    let forest = self.parse_forest(input);
    let trees = forest.trees(&self);
    trees.into_iter().filter_map(|t| {
      unify_tree(t).map(Some).unwrap_or(None)
    }).collect::<Vec<_>>()
  }
}
...
impl FromStr for Grammar {
  ...
  fn from_str(s: &str) -> Result<Self, Self::Err> {
    let (rules, nonterminals) = parse_grammar::parse(s)?;
    if rules.is_empty() {
    ...
    let rules: HashMap<String, Vec<Rc<Rule>>> = rules
      .into_iter()
...
mod parse_grammar {
  ...
  #[derive(Debug, Clone)]
  /// name, features (possibly empty, should be empty if terminal)
  ...
  pub fn parse(s: &str) -> Result<(Vec<Rule>, HashSet<String>), Err> {
    ...
    let rules: Vec<Rule> = rules.into_iter().map(|r| make_rule(r, &nonterminals)).collect::<Result<_, Err>>()?;
  ...
  fn parse_rule(s: &str) -> ParseResult<ParsedRule> {
    ...
    let s = skip_whitespace(s);
    let (_, s) =
      needed_re(&*ARROW, s).map_err(|e| -> Err { format!("rule arrow: {}", e).into() })?;
  ...
  fn make_symbol(s: ParsedSymbolOrTerminal) -> Result<Symbol, Err> {
    let features = s
      ...
      .into_iter()
      .map(|feature| {
        let node: NodeRef = if feature.value == crate::grammar::TOP {
          NodeRef::new_top()
        } else {
  ...
  fn make_rule(r: ParsedRule, nonterminals: &HashSet<String>) -> Result<Rule, Err> {
    // annotate symbol with child-0, child-1, etc. feature groups from productions
    let r = adopt_child_features(r);
...
fn test_parse_rules() {
  ...
  let trees = g.parse(&["he", "said", "that", "they", "kiss", "themselves"]);
